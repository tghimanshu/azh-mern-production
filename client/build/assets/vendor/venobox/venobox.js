    ...
    venobox: function (options) {
      ...
      return this.each(function () {
        ...
        obj.on("click", function (e) {
          ...
          preloader = '<div className="vbox-preloader">';

          switch (option.spinner) {
            case "rotating-plane":
              preloader += '<div className="sk-rotating-plane"></div>';
              break;
            case "double-bounce":
              preloader +=
          ...
          navigation =
            '<a className="vbox-next">' +
            option.htmlNext +
            '</a><a className="vbox-prev">' +
            option.htmlPrev +
            "</a>";
          vbheader =
            '<div className="vbox-title"></div><div className="vbox-left"><div className="vbox-num">0/0</div></div><div className="vbox-close">' +
            option.htmlClose +
            "</div>";
          vbfooter = '<div className="vbox-share"></div>';

          core =
            '<div className="vbox-overlay ' +
            extraCss +
            '" style="background:' +
            overlayColor +
            '">' +
            preloader +
            '<div className="vbox-container"><div className="vbox-content"></div></div>' +
            vbheader +
            navigation +
            vbfooter +
            "</div>";
        ...
        function loadAjax() {
          $.ajax({
            ...
            .done(function (msg) {
              content.html('<div className="vbox-inline">' + msg + "</div>");
              preloadFirst();
            })
            .fail(function () {
              content.html(
                '<div className="vbox-inline"><p>Error retrieving contents, please retry</div>'
              );
              updateoverlay();
            });
        }
        ...
        function loadIframe() {
          content.html(
            '<iframe className="venoframe" src="' + dest + '"></iframe>'
          );
        ...
        function loadVid(autoplay) {
          ...
          content.html(
            '<iframe className="venoframe vbvid" webkitallowfullscreen mozallowfullscreen allowfullscreen allow="autoplay" frameborder="0" src="' +
              player +
              videoObj.id +
              queryvars +
              '"></iframe>'
        ...
        function loadInline() {
          content.html(
            '<div className="vbox-inline">' + $(dest).html() + "</div>"
          );
          updateoverlay();
        }
